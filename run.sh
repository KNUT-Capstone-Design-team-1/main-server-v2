...
if [ $1 = "STAND-ALONE" ]; then
  ...
  docker run -d \
    -v /must_be_madness/what_is_pill/wip-main-server-v2/logs:/usr/local/wip-main-server-v2/logs \
       /must_be_madness/what_is_pill/resources/pill_recognition:/usr/local/resources/pill_recognition \
       /must_be_madness/what_is_pill/resources/drug_permission:/usr/local/resources/drug_permission \
